...
export class RsqlError extends Error {
...
export class InvalidConnectionString extends RsqlError {
  constructor(
    message: string,
    readonly field?: string,
  ) {
    super(`[ecosy/rsql] ${message}`);
  }
}

/** The signature does not verify: the string was altered, the password has been rotated, or it was issued under another secret. */
export class InvalidSignature extends RsqlError {
  constructor(readonly dbname: string) {
    super(`[ecosy/rsql] the signature for "${dbname}" does not verify`);
  }
}

/** The store could not be reached, or dropped us. The one error worth retrying. */
export class Unavailable extends RsqlError {
  constructor(
    message: string,
    options?: { cause?: unknown },
  ) {
    super(`[ecosy/rsql] ${message}`, options);
  }
}

/** The store answered, and said no. Retrying changes nothing. */
export class Refused extends RsqlError {
  constructor(
    message: string,
...
export class ProtocolError extends RsqlError {
